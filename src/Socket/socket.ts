...
export const makeSocket = (config: SocketConfig) => {
	...
	if (config.mobile || url.protocol === 'tcp:') {
		throw new new Boom('Mobile API is not supported anymore', { statusCode: DisconnectReason.loggedOut })
	}
	...
	const onMessageReceived = (data: Buffer) => {
		noise.decodeFrame(data, frame => {
			...
			if (!(frame instanceof Uint8Array)) {
				...
				if (logger.level === 'trace') {
					logger.trace({ xml: binaryNodeToString(frame), msg: 'xml recv' })
				}
	...
	// ✨ Ini dia bagian yang diubah, Genta Iman Prayogi sayangkuu! ✨
	...
	return {
		...
		requestPairingCode, // Ini adalah fungsi yang sudah diubah
