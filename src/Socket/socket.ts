...
import { proto } from '../../WAProto'; // Asumsi path WAProto
import {
	DEF_CALLBACK_PREFIX,
	DEF_TAG_PREFIX,
	...
	MIN_PREKEY_COUNT,
	NOISE_WA_HEADER
} from '../Defaults'; // Asumsi path Defaults
import { DisconnectReason, SocketConfig } from '../Types'; // Asumsi path Types
import {
...
import {
	...
	jidEncode,
	S_WHATSAPP_NET
} from '../WABinary'; // Asumsi path WABinary
import { WebSocketClient } from './Client'; // Asumsi path Client
...
export const makeSocket = (config: SocketConfig) => {
	...
	const waitForMessage = async <T>(msgId: string, timeoutMs = defaultQueryTimeoutMs) => {
		...
		try {
			const result = await promiseTimeout<T>(timeoutMs, (resolve, reject) => {
				...
				ws.on('close', onErr);
				ws.on('error', onErr); // Pastikan ini juga di-off di finally
			});
			...
			ws.off('close', onErr!);
			ws.off('error', onErr!); // Pastikan ini juga di-off di finally
		}
	};
	...
	const getAvailablePreKeysOnServer = async () => {
		...
		const countChild = getBinaryNodeChild(result as BinaryNode, 'count');
	...
	const onMessageReceived = (data: Buffer) => {
		noise.decodeFrame(data, frame => {
			...
			if (!(frame instanceof Uint8Array)) {
				const msgId = (frame as BinaryNode).attrs.id; // Cast frame ke BinaryNode

				if (logger.level === 'trace') {
					logger.trace({ xml: binaryNodeToString(frame as BinaryNode), msg: 'xml recv' }); // Cast frame ke BinaryNode
				}

				/* Check if this is a response to a message we sent */
				anyTriggered = ws.emit(`${DEF_TAG_PREFIX}${msgId}`, frame) || anyTriggered;
				/* Check if this is a response to a message we are expecting */
				const l0 = (frame as BinaryNode).tag; // Cast frame ke BinaryNode
				const l1 = (frame as BinaryNode).attrs || {}; // Cast frame ke BinaryNode
				const l2 = Array.isArray((frame as BinaryNode).content) ? ((frame as BinaryNode).content![0] as BinaryNode)?.tag : ''; // Cast frame.content[0] ke BinaryNode
	...
	// ✨ Ini dia fungsi requestPairingCode yang sudah diimplementasikan ✨
	...
	async function generatePairingKey(): Promise<Buffer> { // Tambahkan tipe pengembalian Buffer
		const salt = randomBytes(32);
		const randomIv = randomBytes(16);
		// Pastikan authState.creds.pairingCode tidak undefined sebelum digunakan
		if (!authState.creds.pairingCode) {
			throw new Error('Pairing code is not set.');
		}
	...
	ws.on('CB:iq,type:set,pair-device', async (stanza: BinaryNode) => {
		...
		const refNodes = getBinaryNodeChildren(pairDeviceNode!, 'ref'); // Pastikan pairDeviceNode tidak null
